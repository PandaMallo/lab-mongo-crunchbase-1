      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "4":
            ...
            db.collection('companies').find({}).sort({number_of_employees: -1}, { name: 1, _id: 0 }).limit(10).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({ $and: [{name:'Facebook'},{'relationship.is_past': false}]}, { _id: 0, 'relationship.person.first_name': 1, 'relationship.person.last_name': 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({tag_list:{$regex:'social-networking'}} , { _id: 0, name: 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({"offices.city": 'London'}, { name: 1, _id: 0, 'offices.London.address1': 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            case "17":
              ...
              db.collection('companies').find({ $and:[{tag_list:{$regex:'social-networking'}},{"offices.city": 'New York'} ,{founded_year:{$gte: 2002}}, {founded_year:{$lte: 2016}}]}).count((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
